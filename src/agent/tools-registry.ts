...
import { getEmails, getDocuments, getDocumentLink, getContacts, getLimsRequests } from './tools/database-tools';
...
export function createTools(): AgentTool[] {
  const tools: AgentTool[] = [
    ...
    },
    {
      schema: {
        ...
        input_schema: {
          type: 'object',
          properties: {
            file_key: { type: 'string', description: 'Ключ файла из карточки документа (get_documents → file_key)' },
          },
          required: ['file_key'],
        },
      },
      execute: (ctx, args) => getDocumentLink(ctx, args),
    },
    {
      schema: {
        name: 'get_contacts',
        description: 'Поиск контактов (доступен, если у пользователя есть права на плагин «Контакты»). Сначала смотрит локальный кэш; если там ничего не нашлось (или кэша нет) — идёт напрямую в БД сервера. query — точное вхождение подстроки (без учёта регистра), НЕ семантический поиск — для русских слов ищи короткой основой без окончания.',
        input_schema: {
          type: 'object',
          properties: {
    ...
    },
    {
      schema: {
        name: 'get_photos',
        description: 'Поиск фотографий в корпоративном фотобанке (доступен, если у пользователя есть права на плагин «LogicTEAM.Фотобанк»). Всегда напрямую из БД сервера — локальный кэш не используется (у разных пользователей разная видимость папок/файлов). Ищи по запросу пользователя («найди фото …»): описание кадра, теги, названия папок. Возвращает карточки: id, title, description, tags, folder_name, file_key и др. Если пользователь сам назвал id фото (например, скопировал его из окна просмотра в плагине) — передай его в поле id для точного поиска вместо query.',
        input_schema: {
          type: 'object',
          properties: {
            ...
            limit: { type: 'number', description: 'Сколько показать (по умолчанию 20, максимум 200)' },
            id: { type: 'number', description: 'Точный id фото, если пользователь явно его назвал (из предыдущего показа списка или скопировал в плагине) — возвращает только эту карточку, query/kind игнорируются' },
          },
        },
      },
