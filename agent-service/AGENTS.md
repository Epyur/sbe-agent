...
- **2026-09-06 — ЮГайл: «мои задачи» без похода за пользователями + агрегация
  ...
  вызвала `get_yougile_tasks` БЕЗ фильтра с `limit:200` — 200 полных сырых
  карточек задач осели в транскрипте чата (история диалога отправляется
  модели целиком на каждое следующее сообщение). Когда чуть позже пользователь
  попросил презентацию с объёмом задач по периодам/исполнителям, раздувшийся
  этим транскрипт + добавленные данные ЛИМС не уложились в тайм-аут LLM —
  упало 504. Тот же класс проблемы, что уже дважды чинили для
  `get_lims_requests` (см. записи 2026-09-05 ниже), только для ЮГайла защиты
  ещё не было вовсе.
  - **`yougileResolveMyID(ctx, email)`**: резолвит id пользователя ЮГайла по
    совпадению email в `/users` (кэш в памяти процесса, `stringCache`, id не
    меняется в отличие от ключа доступа). `GET /api/agent/yougile/tasks?mine=1`
    — сервер сам подставляет `assignedTo=<резолвленный id>`, модели не нужно
    заранее идти за списком пользователей.
  - **`yougileTaskStats(ctx, email, dateFrom, dateTo, groupBy)`**: считает
    поступившие (`timestamp` в диапазоне)/завершённые (`completedTimestamp` в
    диапазоне, только если `completed=true`) задачи по периодам
    (день/неделя-с-понедельника/месяц) + разбивку по каждому исполнителю
    (`created`/`completed`) — по уже полученным данным на сервере, НЕ модель
    вручную по сырым карточкам. `GET /api/agent/yougile/task-stats?date_from=
    &date_to=&group_by=`. Имя/email исполнителя подтягивается из `/users`
    одним дополнительным вызовом.
